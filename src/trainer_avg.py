...
class PROTACTrainer:
    def __init__(self, input_dims_dict=None, config=None):
        ...
        logger.info(f"Initializing Averaging Ensemble (Baseline). Dims: {input_dims_dict}")
        ...
        # Meta-learner removed. Ensemble is now a simple average.
    ...
    def train_all(self, views_train, y_train, views_val=None, y_val=None, retrain_meta=True):
        """Trains experts and optimizes the decision threshold for the average probability."""
        logger.info("Stage 1: Training Heterogeneous Experts...")
        
        if retrain_meta:
            # We still split validation to find an unbiased threshold for the average
        ...
        if not retrain_meta:
            logger.info("Threshold Optimization Skipped: Using pre-optimized threshold.")
            return

        # Stage 2: Averaging & Threshold Optimization
        logger.info("Stage 2: Optimizing Decision Threshold for Averaged Probabilities...")
        
        # Calculate Simple Average on the tuning split
        X_tune_stack = np.column_stack(expert_tune_probs)
        avg_p_tune = np.mean(X_tune_stack, axis=1)
        
        best_t, best_mcc = 0.5, -1
        for t in np.linspace(0.05, 0.95, 91):
            mcc = matthews_corrcoef(self.y_tune, (avg_p_tune >= t).astype(int))
            if mcc > best_mcc: 
                best_mcc, best_t = mcc, t
                
        self.optimized_threshold = float(best_t)
        logger.info(f"Final Optimized Threshold: {self.optimized_threshold:.2f} (Avg Val MCC: {best_mcc:.4f})")
    ...
    def ensemble_predict(self, views_test):
        ...
        # Epistemic uncertainty (std dev between experts) remains relevant
        uncertainties = np.std(X_meta_test, axis=1)
        
        # Compute Simple Average
        final_p = np.mean(X_meta_test, axis=1)
    ...
    def run_exhaustive_upset_study(self, views_val, y_val, views_test, y_test, output_path, logger):
        logger.info("--- RUNNING UPSET STUDY (AVERAGING BASELINE) ---")
        expert_names = list(self.models_list.keys())
        
        tune_probs, test_probs = {}, {}
        for name, model in self.models_list.items():
            full_val_probs = model.predict_proba(views_val[name])[:, 1]
            tune_probs[name] = full_val_probs[self.idx_tune]
            test_probs[name] = model.predict_proba(views_test[name])[:, 1]
        ...
        for r in range(1, len(expert_names) + 1):
            for combo in itertools.combinations(expert_names, r):
                # Calculate average for the subset
                subset_tune_p = np.mean(np.column_stack([tune_probs[n] for n in combo]), axis=1)
                subset_test_p = np.mean(np.column_stack([test_probs[n] for n in combo]), axis=1)
                
                # Re-optimize threshold for the average of this subset
                best_t, best_mcc = 0.5, -1
                for t in np.linspace(0.05, 0.95, 91):
                    mcc = matthews_corrcoef(self.y_tune, (subset_tune_p >= t).astype(int))
                    if mcc > best_mcc: best_mcc, best_t = mcc, t
                
                preds = (subset_test_p >= best_t).astype(int)
    ...
    def save_models(self, output_dir):
        ...
        # Meta learner file omitted; only threshold metadata saved
